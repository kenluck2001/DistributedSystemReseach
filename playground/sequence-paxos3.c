...
int main(int argc, char **argv)
{
    ...
    int rank;
    //client
    struct mpi_counter_t *client_msg_cnts = create_shared_var(row_comm[CLIENT], 0);
    ...
    struct mpi_counter_t *accept_epoch_cnts = create_shared_var(row_comm[ACCEPTOR], 0);
    ...
    if (cur_role == CLIENT) {
        ...
        { 
            ...
            if (flag != 0)
            {
                if (ret == MPI_SUCCESS )
                {
                   ...
                   cnt = increment_counter(client_msg_cnts, 1);
                }
                flag = -1;
            }
            if (cnt== threshold)
            {
                ...
                cnt = reset_var(client_msg_cnts, 0);
                flag = -1;
                ready = 0;
                goto RESTART_CLIENT;
                //break;
        ...
        //RESTART_PROPSER: {start = MPI_Wtime(); ready = 1;}


        RESTART_PROPSER: {
            start = MPI_Wtime();
            printf("get_var(prop_list_last_pos): %d, recv.total_size: %d\n", get_var(prop_list_last_pos), recv.total_size);
            if (recv.total_size == 0) {ready = 1;}
            else if ((get_var(prop_list_last_pos) <= recv.total_size-1) && (recv.total_size !=0 )) {ready = 1;}
            else {printf("ENDED\n");ready = 0;}
        }
...
//printf("get_var(prop_list_last_pos): %d, recv.total_size: %d\n", get_var(prop_list_last_pos), recv.total_size);
        ...
        RESTART_ACCEPTOR:  {
            printf("get_var(accept_epoch_cnts): %d, recv.total_size: %d\n", get_var(accept_epoch_cnts), recv.total_size);
            increment_counter(accept_epoch_cnts, 1);
            if (recv.total_size == 0) {ready = 1;}
            else if ((get_var(accept_epoch_cnts) <= recv.total_size) && (recv.total_size !=0 )) {ready = 1;}
            else {printf("ENDED\n");ready = 0;}
        }
    ...
    delete_counter(&client_msg_cnts);

    delete_counter(&accept_epoch_cnts);
