...
void printArray(int *array, int size){
   char * description [3] = {"source", "destination", "time"};

   for(int loop = 0; loop < size; loop++)
      printf("%s: %d | ", description[loop], array[loop]);
   printf("\n");
}
...
int main(int argc, char **argv)
{
    ...
    if (my_rank == 0) {
        ...
        buf[0] = my_rank;   //source
        buf[1] = 0;         //destination
        buf[2] = 1;         //time

        for (other_rank = 0; other_rank < num_procs; other_rank++)
        {
            //MPI_Send(buf, size, MPI_INT, other_rank, my_rank, MPI_COMM_WORLD);
            MPI_Send(buf, size, MPI_INT, other_rank, my_rank, MPI_COMM_WORLD);
        }
        ...
        for (other_rank = 0; other_rank < num_procs; other_rank++)
        {
            MPI_Recv(buf, size, MPI_INT, other_rank, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            printArray(buf, size);
        }
        ...
        MPI_Recv(buf, size, MPI_INT, source, tag, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
