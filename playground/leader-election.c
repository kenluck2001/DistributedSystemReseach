...
void checkLeader (data recv, int num_procs, int leader, MPI_Request requests [], MPI_Datatype mpi_data_type, int *max_ballot, int *p_cnt)
{
    int promise_cnt;
    enum msgTag ctag;
    data accept_value;
    ...
    *p_cnt = *p_cnt + 1;
    ...
    printf ("num_procs: %d, promise_cnt: %d\n", num_procs, promise_cnt);

    // for possible extensions due to variations of implementing bully algorithm
    //if ((*p_cnt)==promise_cnt && (leader != accept_value.pid))
    //if ((*p_cnt)==promise_cnt)

    //for testing use if ((*p_cnt)==1)
    if ((*p_cnt)==1 && (leader != accept_value.pid))
    {
        ctag = mSetLeader;
    ...
    { 
        ...
        if (flag != 0)
        {
            if (ret == MPI_SUCCESS )
            {
                ...
                if (tag == mElection)
                {
                    checkLeader(recv, num_procs, (*leader), requests, mpi_data_type, &max_ballot, &p_cnt);
