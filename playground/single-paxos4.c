...
void modeOp ( int *, int *, int *, MPI_Datatype *);
int getMode (int *, int);
struct mpi_counter_t *create_shared_var(MPI_Comm, int);
int near_atomic(struct mpi_counter_t *, int, int []);
int near_atomic_shared(struct mpi_counter_t *, int, int [], MPI_Op );
int modify_var(struct mpi_counter_t *, int, MPI_Op );
int increment_counter(struct mpi_counter_t *, int );
void delete_counter(struct mpi_counter_t **);
int getRole (int, int, int);
...
void modeOp(int *invec, int *inoutvec, int *len, MPI_Datatype *dtype)
{
    //int mode = getMode (invec, *len);
...
int getMode (int *arr, int length)
{
    ...
    int maxVal = getMax(arr, length);
    int minVal = getMin(arr, length);

    int range = maxVal - minVal + 1;

    //printf ("maxVal: %d, minVal: %d, range: %d\n", maxVal, minVal, range);
    int map[range];
...
int near_atomic_shared(struct mpi_counter_t *count, int val, int vals[], MPI_Op op) {
    MPI_Win_lock(MPI_LOCK_EXCLUSIVE, count->hostrank, 0, count->win);
    for (int i=0; i<count->size; i++) {
        if (i == count->rank) {
            MPI_Accumulate(&val, 1, MPI_INT, 0, i, 1, MPI_INT, op, count->win);
        } else {
            MPI_Get(&vals[i], 1, MPI_INT, 0, i, 1, MPI_INT, count->win);
        }
    }
...
int modify_var(struct mpi_counter_t *count, int valuein, MPI_Op op) {
    int *vals = (int *)malloc( count->size * sizeof(int) );
    int val;
    near_atomic_shared(count, valuein, vals, op);
...
int main(int argc, char **argv)
{
    ...
    MPI_Op op;
    MPI_Op_create( (MPI_User_function *)modeOp, 1, &op );
            ...
            if (flag != 0)
            {
                if (ret == MPI_SUCCESS )
                {
                    ...
                    if (tag == mPROPOSE)
                    {
                        ...
                        round_number = modify_var(prop_round_number, recv.round_number, op);
                        current_value = modify_var(prop_current_value, recv.value, op);
                        max_promise_round_number = modify_var(prop_max_promise_round_number, recv.round_number, op);
            ...
            if (flag != 0)
            {
                if (ret == MPI_SUCCESS )
                {
                    ...
                    if (tag == mPREPARE)
                    {
                        ...
                        {
                            round_number_promise = modify_var(accept_round_number_promise, recv.round_number, op);
                            ctag = mPROMISE;
                            ...
                            //{
                                current_value_accepted = modify_var(accept_current_value_accepted, recv.value, op);
                            //}
                            payload.value = current_value_accepted; // current value

                            //if (round_number_accepted == EMPTY)
                            //{
                                round_number_accepted = modify_var(accept_round_number_accepted, recv.round_number, op);
            ...
            if (flag != 0)
            {
                if (ret == MPI_SUCCESS )
                {
                    ...
                    if (tag == mDECIDE)
                    {
                        ...
                        {
                            current_value_decided = modify_var(learner_current_value_decided, recv.value, op);
                        }
                    }
                    //cnt += 1;
