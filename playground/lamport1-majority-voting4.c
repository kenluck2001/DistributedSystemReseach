    ...
    //while (1) {
        ...
        if (my_rank == 0) {
            ...
            for (other_rank = 0; other_rank < num_procs; other_rank++)
            {
                ...
                MPI_Issend(lamport_vec, num_procs, MPI_INT, other_rank, my_rank, MPI_COMM_WORLD, &requests[other_rank]);
            ...
            /**            
            memset( rounds, 0, num_procs * sizeof(int) );

            int ret = MPI_Irecv(rounds, num_procs, MPI_INT, 0, 0, MPI_COMM_WORLD, &requests[num_procs]);
            if (ret == MPI_SUCCESS )
            {   
                ...
                int cnt = 0;
                for(int loop = 0; loop < num_procs; loop++)
                {
                    printf("a %d ", rounds[loop]);
                    cnt += rounds[loop];
                }
                if (cnt == 0)
                {
            ...
            */
            ...
            { 
                ...
                if (ret == MPI_SUCCESS )
                {
                    ...
                    MPI_Issend(lamport_vec, num_procs, MPI_INT, other_rank, 0, MPI_COMM_WORLD, &requests[other_rank]);
                }
