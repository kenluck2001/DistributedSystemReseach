...
int near_atomic_shared(struct mpi_counter_t *, int, int [] );
int modify_var(struct mpi_counter_t *, int);
...
int near_atomic_shared(struct mpi_counter_t *count, int val, int vals[]) {
    MPI_Win_lock(MPI_LOCK_EXCLUSIVE, count->hostrank, 0, count->win);
    for (int i=0; i<count->size; i++) {
        if (i == count->rank) {
            MPI_Accumulate(&val, 1, MPI_INT, 0, i, 1, MPI_INT, MPI_MAX, count->win);
        } else {
            MPI_Get(&vals[i], 1, MPI_INT, 0, i, 1, MPI_INT, count->win);
        }
    }
...
int modify_var(struct mpi_counter_t *count, int valuein) {
    ...
    val = getMode (vals, count->size); // fail for too much storage
    //printf("mode: %d", mode);
    free(vals);
    return val;
}
