...
This is a demonstrations of sequence paxos as a repeated run of single paxos on
a list of value while maintain the order with a series of states in each role.
...
void clientLogic(MPI_Comm row_comm[], MPI_Request requests[],
    ...
    int threshold =
        NUM_REQUESTS;  // Number of expected decision messages before
                       // potentially moving to the next sequence element
    ...
    while (processing) {
        if (recv_last_pos <= data_size - 1) {
            ...
            for (int other_rank = start; other_rank < end; other_rank++) {
                ...
                printf(
                    "Client %d: Sent propose for sequence[%d] = %d to proposer "
                    "%d\n",
            ...
            // Wait to receive the decided sequence position (threshold number
            // of times)
            while (cnt < threshold) {
                if (flag != 0) {
                    tag = mDECIDE_SEQ;
                    ret = MPI_Irecv(&recv_last_pos, 1, MPI_INT, MPI_ANY_SOURCE,
                                    tag, row_comm[CLIENT], &requests[my_rank]);
                    flag = 0;
                }
                MPI_Test(&requests[my_rank], &flag, &status[my_rank]);
                if (flag != 0 && ret == MPI_SUCCESS) {
                    printf(
                        "Client %d: Received recv_last_pos: %d (count: %d)\n",
            ...
            // After receiving enough decision messages, continue to the next
            // sequence element
        } else {
            processing = 0;  // Exit the main loop when all sequence elements
                             // are processed
        }
    }
...
void proposerLogic(MPI_Comm row_comm[], MPI_Request requests[],
                   MPI_Status status[], MPI_Datatype mpi_data_type,
                   MPI_Datatype mpi_list_type, int my_rank, int num_procs,
                   int nRole) {
    int NUM_REQUESTS = num_procs / nRole;
    ...
    while (ready) {
        ...
        while (cnt < threshold && ready) {
            ...
            if (flag != 0) {
                if (ret == MPI_SUCCESS) {
                    ...
                    printf(
                        "Proposer %d: Received value: %d, round: %d, tag: %d, "
                        "last_pos: %d\n",
                    ...
                    if (tag == mPROPOSE) {
                        ...
                        for (int other_rank = start_acceptor;
                             other_rank < end_acceptor; other_rank++) {
                            MPI_Isend(&recv, 1, mpi_data_type, other_rank,
                                      prepare_tag, row_comm[ACCEPTOR],
                                      &requests[other_rank]);
                        }
                        ...
                        if (recv.custom_round_number == round_number) {
                            ...
                            if (promise_cnt ==
                                (int)MAX((NUM_REQUESTS / 2.0), 1)) {
                                int start_acceptor = ACCEPTOR * NUM_REQUESTS;
                                int end_acceptor =
                                    start_acceptor + NUM_REQUESTS;
                                if (accept_value.value == EMPTY) {
                                    accept_value.value = current_value;
                                }
                                accept_value.custom_round_number =
                                    round_number;  // Ensure round number is set
                                                   // for accept
                                for (int other_rank = start_acceptor;
                                     other_rank < end_acceptor; other_rank++) {
                                    MPI_Isend(&accept_value, 1, mpi_data_type,
                                              other_rank, accept_tag,
                                              row_comm[ACCEPTOR],
                                              &requests[other_rank]);
                                }
                            }
                        ...
                        if (recv.custom_round_number == round_number) {
                            ...
                            if (acks == (int)MAX((NUM_REQUESTS / 2.0), 1)) {
                                ...
                                for (int other_rank = start_learner;
                                     other_rank < end_learner; other_rank++) {
                                    MPI_Isend(&list_recv, 1, mpi_list_type,
                                              other_rank, decide_tag,
                                              row_comm[LEARNER],
                                              &requests[other_rank]);
                                }
                            }
                        ...
                        printf(
                            "Proposer %d: Received NACK for round %d, current "
                            "round %d\n",
                            my_rank, recv.custom_round_number, round_number);
                        if (recv.custom_round_number == round_number) {
                            break;  // Break inner loop to restart proposal with
                                    // a new round number
                        }
                    }
                    cnt += 1;
        ...
        // The outer loop condition will determine if we continue to the next
        // proposal based on the received recv.total_size and last_pos.
    }
...
void acceptorLogic(MPI_Comm row_comm[], MPI_Request requests[],
    ...
    while (ready) {
        ...
        if (recv.total_size == 0) {
            ready = 1;
        } else if ((accept_epoch_cnts <= recv.total_size) &&
                   (accept_epoch_cnts >= 0)) {
            ready = 1;
        } else {
                    ...
                    if (tag == mPREPARE) {
                        ...
                        if (round_number_promise <= recv.round_number) {
                            ...
                            payload.round_number =
                                round_number_accepted;  // current rounder
                                                        // number
                            ...
                            for (int other_rank = start; other_rank < end;
                                 other_rank++) {
                                MPI_Isend(&payload, 1, mpi_data_type,
                                          other_rank, ctag, row_comm[PROPOSER],
                                          &requests[other_rank]);
                            }
                        } else {
                            ctag = mNACK;
...
void learnerLogic(MPI_Comm row_comm[], MPI_Request requests[],
    ...
    while (processing) {
        ...
        printf(
            "Learner %d: Waiting for decide messages (current last_pos: %d)\n",
        ...
        while (cnt < threshold) {
            if (flag != 0) {
                ret = MPI_Irecv(&recv, 1, mpi_list_type, MPI_ANY_SOURCE,
                                mDECIDE, row_comm[LEARNER], &requests[my_rank]);
                flag = 0;
            }
            MPI_Test(&requests[my_rank], &flag, &status[my_rank]);
        ...
        printf(
            "Learner %d: Received %d decide messages (threshold: %d), current "
            "last_pos: %d\n",
        ...
        for (int other_rank = start_client; other_rank < end_client;
             other_rank++) {
            MPI_Isend(&saved_recv.last_pos, 1, MPI_INT, other_rank, tag_client,
                      row_comm[CLIENT], &requests[other_rank]);
        ...
        if (saved_recv.last_pos < NUM_OF_ELEM) {
            continue;  // Go back to the beginning of the while loop to receive
                       // more decisions
        } else {
...
int main(int argc, char **argv) {
    ...
    if (cur_role == CLIENT) {
        // int nRole = 4;
        clientLogic(row_comm, requests, status, mpi_data_type, my_rank,
                    num_procs, nRole);
    } else if (cur_role == PROPOSER) {
        // int nRole = 4;
        proposerLogic(row_comm, requests, status, mpi_data_type, mpi_list_type,
                      my_rank, num_procs, nRole);
        ...
        // int nRole = 4;
        learnerLogic(row_comm, requests, status, mpi_data_type, mpi_list_type,
                     my_rank, num_procs, nRole);
    }
