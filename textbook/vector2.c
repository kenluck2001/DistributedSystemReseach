...
This is a demonstrations of how to use a vector clock to catch happen-before
...
int main(int argc, char **argv) {
    ...
    if (my_rank == 0) {
        ...
        MPI_Send(lamport_vec, num_procs, MPI_INT, next_rank, my_rank,
                 MPI_COMM_WORLD);
        ...
        for (int k = 0; k < num_procs; k++) {
            lamport_vec[k] =
                MAX(lamport_vec[k] + 1, rec_lamport_vec[k]);  // time
        }
