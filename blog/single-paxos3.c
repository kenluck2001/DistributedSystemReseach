...
This is a demonstrations of single paxos works, we allow the users to group the processes into 4 roles  (CLIENT, PROPOSER, ACCEPTOR, LEARNER), where each role has the same number of processes. We allow multiple of 4 for the total number of processes and each role has a set of shared state handled with our custom distributed shared primitives.

How to run the source code
===========================
mpicc single-paxos3.c && mpiexec -n 4 ./a.out
mpicc single-paxos3.c && mpiexec -n 8 ./a.out
mpicc single-paxos3.c && mpiexec -n 12 ./a.out
