...
This is a demonstrations of how to use a lamport clock to catch happen-before event in messages sents over the channel.
...
int main(int argc, char **argv)
{
    ...
    if (my_rank == 0) {
        ...
        {
            MPI_Send(lamport_vec, num_procs, MPI_INT, other_rank, my_rank, MPI_COMM_WORLD);
        }

        /* Receive messages from all other process */
        for (other_rank = 0; other_rank < num_procs; other_rank++)
        {
            MPI_Recv(lamport_vec, num_procs, MPI_INT, other_rank, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
