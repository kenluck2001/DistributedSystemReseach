...
This is a demonstrations of how to use a lamport clock to use majority voting to build a distributed key value hash map.
...
int main(int argc, char **argv)
{
    ...
    if (my_rank == 0) {
        ...
        createMapArray( &map_array, TABLESIZE, EMPTY ); // initialize the map here
        ...
        setKeyValueToMap( &map_array, package.key, package.value ); // add key, value pair to the map
            ...
            if (flag != 0)
            {
                if (ret == MPI_SUCCESS )
                {
                    ...
                    if (largest_timestamp < recv.lamport_vec[source] ){
                        ...
                        setKeyValueToMap( &map_array, recv.key, recv.value ); // add key, value pair to the map
                    }
                    if (source != my_rank){
        ...
        {
            printf("For key: %s, retrieved value: %i, available in map at rank: %d\n", key, store_Value, my_rank);
        } else {
